...
vec3 LightCalculation(int idx){
	...
	if(u_lightInfo[idx].u_lightType == 0)
	{
		float shadow = ShadowCalculation(v_PosFromLight[idx], idx);
		vec3 lighting = (ambient + (1.0 - shadow) * (diffuse + specular)) * fragColor;
		return lighting;
	}
	else if(u_lightInfo[idx].u_lightType == 1)
	{
		float theta     = dot(-lightDir, normalize(u_lightInfo[idx].u_lightDir));
		float epsilon   = u_lightInfo[idx].u_cutOff - 0.993;
		float intensity = (theta - 0.993) / epsilon;
		
		if(theta < 0.993)
		{
			return ambient * fragColor;
		}
		else if(theta < u_lightInfo[idx].u_cutOff && theta > 0.993)
		{
			diffuse  *= intensity;
			specular *= intensity;
